...
while cap.isOpened():
    ...
    # Draw ROI polygon
    cv2.polylines(vis_frame, [roi_points], isClosed=True, color=(0, 255, 0), thickness=2)
    ...
    for r in results:
        ...
        for box in boxes:
            ...
            if best_conf >= 0.95:
                ...
                # Put text on visualization frame
                cv2.putText(vis_frame, f"{best_text} ({best_conf:.2f})", (x1, y1-10), 
                           cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0, 255, 0), 2)

    # Show the frame with visualizations
    cv2.imshow('License Plate Detection', vis_frame)
